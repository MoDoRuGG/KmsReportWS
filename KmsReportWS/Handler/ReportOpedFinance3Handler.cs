...
{
    public class ReportOpedFinance3Handler : BaseReportHandler
    {
        private static readonly Logger Log = LogManager.GetCurrentClassLogger();
        private string _themeName = "oped_finance3";


        public ReportOpedFinance3Handler(ReportType reportType)
          : base(reportType)
        {
        ...
        {
            var report = inReport as ReportOpedFinance3 ??
        ...
        protected List<Report_OpedFinance> MapReport(ReportOpedFinance3 report, int idReportData)
        {
        ...
        {
            var outReport = new ReportOpedFinance3 { ReportDataList = new List<ReportOpedFinance3Data>() };
            ...
            foreach (var themeData in rep.Report_Data)
            {
                ...
                outReport.ReportDataList.AddRange(themeData.Report_OpedFinance.Select(x => new ReportOpedFinance3Data
                {
                    RowNum = x.row_num,
        ...
        protected override void UpdateReport(LinqToSqlKmsReportDataContext db, AbstractReport inReport)
        {
            ...
            foreach (var row in report.ReportDataList)
            {
                ...
                {
                    var rowIns = new ReportOpedFinance3Data
                    {
                        RowNum = row.RowNum,
